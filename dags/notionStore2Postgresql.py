...
NOTION_DATABASE_STORE_ID = Variable.get("NOTION_DATABASE_STORE_ID")
        ...
        if 'properties' in item:
            ...
            for key, value in properties.items():
                ...
                if value['type'] == 'rich_text':
                    ...
                    # 提取 Social Image 的 URL
                    properties_dict[key]['value'] = ', '.join([f.get('external', {}).get('url', '無法獲取 URL') for f in value['files']])
                elif value['type'] == 'created_time':
                    properties_dict[key]['value'] = value['created_time']
...
def save_to_postgresql(input_csv_path):
    ...
    df.to_sql('store', postgres_engine, if_exists='append', index=False)
    print("Data saved to PostgreSQL table 'store'")
...
with DAG(
    ...
    output_json_path = f"/opt/airflow/data/{NOTION_DATABASE_STORE_ID}.json"
    output_csv_path = f"/opt/airflow/data/{NOTION_DATABASE_STORE_ID}.csv"
    transformed_csv_path = f"/opt/airflow/data/{NOTION_DATABASE_STORE_ID}_transformed.csv"

    save_json_task = PythonOperator(
        task_id="save_json",
        python_callable=read_and_save_json,
        op_kwargs={
            "database_id": NOTION_DATABASE_STORE_ID, 
            "headers": headers, 
