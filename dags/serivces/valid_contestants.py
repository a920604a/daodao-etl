...
def get_valid_contestants(session):
    ...
    names_list_str  = Variable.get("paid_names_list")
    names_list = json.loads(names_list_str)
...
def get_marthon_user_list(session):
    result = session.query(User.id).join(Project, Project.user_id == User.id).all()
    user_ids = [row[0] for row in result]
    return sorted(user_ids)

def set_player_role_id(session, user_list : List, role_id = 3):
    session.execute(
