...
class NotionToPostgresETL:
    ...
    def extract_property_value(self, value):
        if value['type'] == 'rich_text':
            ...
            return ', '.join([f.get('external', {}).get('url', '無法獲取 URL') for f in value['files']])
    ...
    def save_to_postgresql(self, csv_path):
        ...
        df.to_sql(self.postgres_table_name, self.postgres_engine, if_exists='append', index=False)
...
with DAG(
    ...
    notion_resource_etl = NotionToPostgresETL(Variable.get("NOTION_DATABASE_RESOURCE_ID"), "resource")
...
with DAG(
    ...
    notion_store_etl = NotionToPostgresETL(Variable.get("NOTION_DATABASE_STORE_ID"), "store")
